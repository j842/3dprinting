...
function jtags3.gettagsize()
   return v(120,3.5,35)
end

function jtags3.gettagborder()
  return 2
end

-------------------------------------------------------

function jtags3.gettagshape()
  local ts=jtags3.gettagsize()
  local g=jtags3.gettagborder()
...
function jtags3.gettagshape_hole()
  -- enlarge it a bit.
  return scale(1.002,1.1,1)*jtags3.gettagshape()
end

-------------------------------------------------------
function jtags3.nametag(text,image)

--text='Griff'
--image='griffbw.jpegW24H25T3V4B2A0C0NS.stl'

local tsize=jtags3.gettagsize()
local w=tsize.x
local t=tsize.y
local h=tsize.z
local e=jtags3.gettagborder()
local textgap=3
...
tag=intersection(tag,jtags3.gettagshape())
...
function jtags3.flatyrect(offset,x,z)
    return {
...
function jtags3.holder()
  ...
  -- remove slot for tag
  h=difference(h,translate(0,hs.y/2-ts.y/2-g,hs.z-ts.z)*jtags3.gettagshape_hole()) --cube(ts))
  ...
  s2=jtags3.flatyrect(v(0,(hs.y-ts.y)/2,2*g),hs.x-4*g,hs.z-3*g)
